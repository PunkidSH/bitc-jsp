...
<%--    getMethod() : 클라이언트가 서버에 데이터 요청 시 데이터를 정달한 방식을 출력 --%>
    <li class="list-group-item">데이터 전송 방식 : <%= request.getMethod()%></li>
<%--  URL과 URI의 차이 : URL은 네트워크 상에서 자원이 어디 있는지 위치를 알려주기 위한 규약, URI는 인터넷에 있는 자원을 어디에 있는지 자원 자체를 식별하는 방법, 현재는 거의 같은 의미로 사용됨--%>
...
<%--  getServerName() : 현재 접속한 서버에서 사용하는 port 번호를 출력--%>
...
<%--  getQueryString() : URI 뒤에 있는 ? 기호와 함께 전달된 데이터를 의미, GET 방식 사용 시 URL과 함께 전잘된 데이터를 출력, POST 방식 사용 시 쿼리스트링은 null 임 --%>
