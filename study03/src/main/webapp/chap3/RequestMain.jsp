...
//     removeAttribute() : 지정한 영역에서 지정한 key를 가지고 있는 데이터 삭제, 지정한 키가 없어도 오류 나지 않음
  ...
  <%
//    getAttribute() : 지정한 영역에서 지정한 key를 가지고 있는 데이터를 출력, 지정한 key가 없으면 null을 출력
...
<%--//    forward() : 지정한 페이지로 이동--%>
<%--    request.getRequestDispatcher("RequestForward.jsp?paramHan=한글&paramEng=eng").forward(request, response);--%>
<%--  %>--%>
