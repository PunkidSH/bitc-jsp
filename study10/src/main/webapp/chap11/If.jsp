...
<%-- jstl의 set을 사용하여 page 영역에 변수 추가--%>
...
<%--      조건식에 ${} 를 사용하지 않고 직접 값을 입력, 무조건 false로 출력 --%>
      ...
      <c:if test="100" var="result3">
        <p>EL이 아닌 정수를 지정하면 무조건 false</p>
      </c:if>
      <p>result3 : ${result3}</p>

      <br>

      <h4>if 문 사용 시 EL 언어로 직접 정수만 사용 시</h4>
<%--  jstl의 if 문 사용 시 정수를 el언어로 표현 시 오류 발생--%>
<%--  JSTL의 if 문 사용 시 test 속성에 조건식을 사용하지 않고 정수만 입력 시 오류 발생 --%>
<%--      <c:if test="${100}" var="result3">--%>
<%--        <p>EL언어를 사용하면?</p>--%>
<%--      </c:if>--%>
<%--      <p>result3 : ${result3}</p>--%>
